...
lme = fitlme(all_subj_data,'BundleValue ~ LItemValue*Type + RItemValue')
X=designMatrix(lme);
X=[X X(:,3).*X(:,4) X(:,3).*X(:,5) X(:,3).*X(:,6)];
fitlmematrix(X,all_subj_data.BundleValue,[],all_subj_data.Subject)
