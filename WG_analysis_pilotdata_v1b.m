...
function LinearModels=WG_analysis_pilotdata_v1b(subjectsToAnalyze)
...
set(gcf,'Paperpositionmode','auto','Papersize',[20 20]);
subplot(2,3,2)
LM_leftright=fitlm(bdm_bundle_ItemValue,bdm_bundle_value,'VarNames',{'LeftItemValue','RightItemValue','BundleValue'},'Intercept',false)
...
text(0.5,18,sprintf('R2: %0.3f \n\\beta1: %0.3f \n\\beta2: %0.3f',LM_leftright.Rsquared.Ordinary, beta(1), beta(2)));
ylim([0 20]);
try
...
LM_mixedbundle=fitlm(bdm_mixedbundle_itemValues,bdm_mixedbundle_value,'VarNames',{'FoodItemValue','TrinketItemValue','BundleValue'},'Intercept',false)
...
text(0.5,18,sprintf('R2: %0.3f \n\\beta1: %0.3f \n\\beta2: %0.3f',LM_mixedbundle.Rsquared.Ordinary, beta(1), beta(2)));
ylim([0 20]);
try
    xlim([0 max(PredictedValues)]);
catch
...
for bdm_class=1:2
    LM_class_leftright{bdm_class}=fitlm(bdm_bundle_ItemValue(bdm_bundle_class==bdm_class,:),bdm_bundle_value(bdm_bundle_class==bdm_class),'VarNames',{'LeftItemValue','RightItemValue','BundleValue'},'Intercept',false)
    fprintf('%s R2 value: %f \n',bdm_class_names{bdm_class+1}, LM_class_leftright{bdm_class}.Rsquared.Ordinary);
    ...
    text(0.5,18,sprintf('R2: %0.3f \n\\beta1: %0.3f \n\\beta2: %0.3f',LM_class_leftright.Rsquared.Ordinary, beta(1), beta(2)));
    ylim([0 20]);
    try
        xlim([0 max(PredictedValues)]);
    catch
    ...
    try
        LM_class_same=fitlm(bdm_bundle_ItemValue(bdm_bundle_class==bdm_class & bdm_bundle_same,1),bdm_bundle_value(bdm_bundle_class==bdm_class & bdm_bundle_same),'VarNames',{'ItemValue','BundleValue'},'Intercept',false)
        fprintf('%s R2 value: %f \n',bdm_class_names{bdm_class+1}, LM_class_same.Rsquared.Ordinary);
        beta=LM_class_same.Coefficients.Estimate;
        PredictedValues=feval(LM_class_same,bdm_bundle_ItemValue(bdm_bundle_class==bdm_class & bdm_bundle_same,1));
        ...
        text(0.5,18,sprintf('R2: %0.3f \n\\beta1: %0.3f',LM_class_same.Rsquared.Ordinary, beta(1)));
        ylim([0 20]);
        try
            xlim([0 max(PredictedValues)]);
        catch
