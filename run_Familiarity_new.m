function run_Familiarity_new(subID)
%%
% run_BDM('150424a')
...
try
    ...
    for i = 1:num_trials
        ...
        if item_rnd_idx(i)<71
            shown_item = ['data/imgs_food/item_',num2str(item_rnd_idx(i)),'.jpg'];
        else
            shown_item = ['data/imgs_trinkets/item_',num2str(item_rnd_idx(i)-100),'.jpg'];
        end
        itm_img = DispImage('init', wpt, shown_item, [0,-h/15], w/50, [100,100]);
        
        target = ceil(rand() * length(w_bin));
        init_valueBDM = 5 * (target - 1) / (length(w_bin) - 1);
        ...
        valueBDM = 5 * (target - 1) / (length(w_bin) - 1); % disp(['$',num2str(valueBDM)]);
