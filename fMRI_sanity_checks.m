...
sbplot3 = subplot(2,2,3);
pos1 = get(sbplot3, 'Position'); % gives the position of current sub-plot
new_pos1 = pos1 +[-0.02 0 0.05 0.05];
set(sbplot3, 'Position',new_pos1 ) % set new position of current sub - plot
PredictedValues=feval(LM_leftright,[bdm_bundle_item_values]);
%plot(PredictedValues,bdm_bundle_value,'.','MarkerSize',20);
binscatter(PredictedValues,bdm_bundle_value,[20 20])
axis equal
hold on
...
pos1 = get(sbplot4, 'Position'); % gives the position of current sub-plot
new_pos1 = pos1 +[-0.02 0 0.05 0.05];
set(sbplot4, 'Position',new_pos1 ) % set new position of current sub - plot
PredictedValues=feval(LM_foodtrinket,[bdm_mixedbundle_item_values]);
%plot(PredictedValues,bdm_mixedbundle_value,'.','MarkerSize',20);
binscatter(PredictedValues,bdm_mixedbundle_value,[20 20])
axis equal
hold on
...
yticks([0:2:20])
if intercept
    text(12,4.5,sprintf('R2 value: %0.3f \n\\beta1 (intercept): %0.3f \n\\beta2 (left): %0.3f \n\\beta3 (right): %0.3f', LM_foodtrinket.Rsquared.Adjusted, beta(1), beta(2), beta(3)),'FontSize',14);
else
    text(12,4.5,sprintf('R2 value: %0.3f \n\\beta1 (left): %0.3f \n\\beta2 (right): %0.3f', LM_foodtrinket.Rsquared.Adjusted, beta(1), beta(2)),'FontSize',14);
end
