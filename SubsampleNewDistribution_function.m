function distribution_measures=SubsampleNewDistribution_function(subID,method)
...
if method==1
    ...
    for bin=1
        ...
        maxValue_in_bin(bin+1)=minValue_in_bin(bin)-1;
    ...
    binsize=minValue_in_bin(1)/4;
    maxValue_in_bin(3:5)=maxValue_in_bin(2)-binsize:-binsize:maxValue_in_bin(2)-3*binsize;
    minValue_in_bin(2:5)=minValue_in_bin(1)-binsize:-binsize:0;
    
    for bin=2:5
        Objects_food=datasample(food_value_matrix(food_value_matrix(:,2)>=minValue_in_bin(bin) & food_value_matrix(:,2)<=maxValue_in_bin(bin),1),...
            min(nnz(food_value_matrix(:,2)>=minValue_in_bin(bin) & food_value_matrix(:,2)<=maxValue_in_bin(bin)),4),'Replace',false);
        Objects_trinkets=datasample(trinket_value_matrix(trinket_value_matrix(:,2)>=minValue_in_bin(bin) & trinket_value_matrix(:,2)<=maxValue_in_bin(bin),1),...
            min(nnz(trinket_value_matrix(:,2)>=minValue_in_bin(bin) & trinket_value_matrix(:,2)<=maxValue_in_bin(bin)),4),'Replace',false);
        
        bin_objects_trinket{bin}=trinket_value_matrix(ismember(trinket_value_matrix(:,1),Objects_trinkets),:);
        trinket_value_matrix(ismember(trinket_value_matrix(:,1),bin_objects_trinket{bin}(:,1)),:)=[];
        bin_objects_food{bin}=food_value_matrix(ismember(food_value_matrix(:,1),Objects_food),:);
        food_value_matrix(ismember(food_value_matrix(:,1),bin_objects_food{bin}(:,1)),:)=[];
...
measures_labels={'Food median','Trinket median','Food mean','Trinket mean','Excess kurtosis','skewness','variance'};
distribution_measures=[median(trinket_values) median(food_values) mean(trinket_values) mean(food_values) kurtosis([trinket_values; food_values])-1.8 skewness([trinket_values; food_values]),...
